...
class FootballSkillsEnv(gym.Env):
    ...
    def __init__(self, render_mode='gif', grid_size = 20, degrade_pitch=False, degrade_mode="sigmoid"):
        super().__init__()
    ...
    def get_gif(self, policy, seed = 20, filename = "output.gif"):
        ...
        while not done and steps < max_steps:
            ...
            if self.degrade_pitch:
                ...
                action = policy[s_index,time_step]
            else:
                s_index = self.state_to_index(obs)
    ...
    def get_gif_special(self, policy, seed = 20, filename = "output.gif"):
        obs, _ = self.reset(seed = seed)
        done = False
        steps = 0
        max_steps = 40
